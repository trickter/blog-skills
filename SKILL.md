---
name: blog-writer
description: Write Chinese technical blog posts from a topic brief or daily hot trends. Use when you want 5 blog topic ideas first, want to learn style from the local blog at http://127.0.0.1:5000, want stronger titles and openings, or want to back up finished posts to the blog-backup GitHub repository.
...
Write Chinese technical blog posts in the style of the local blog, starting from either daily hot trends or a user-provided topic. Always propose 5 topic options with creation advice before writing the article. After the user picks one option, write the post, save it in the backup repo format, and back it up with git.
...
### Step 8: Produce backup-ready markdown
...
### Step 9: Back up with git

Back up the post to `https://github.com/trickter/blog-backup`.

Assumptions:

- Git credentials are already configured on the machine
- The target branch is `main`

Workflow:

1. Ensure a local `blog-backup` clone exists
2. If not present, clone it
3. Save the markdown file into `posts/`
4. Run `git add`
5. Commit with a clear message such as `Add post: <slug>`
...
- The post is committed and pushed unless a concrete git failure prevents it
